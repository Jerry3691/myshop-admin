...
export class EmailComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.emailGroup.patchValue({
      consultant_email: this.data.consultant_email,
      consultant_name: this.data.consultant_name,
    ...
    if (this.data.amount) {
      ...
      const control = voucherArray.at(0);
      control.patchValue({ value: this.data.amount, quantity: 1 });
    }
  }
  ...
  onSubmit() {
    ...
    const payload = {
      ...
      type: this.data.type,
      request_voucher_id: this.data.request_voucher_id || null
    }
