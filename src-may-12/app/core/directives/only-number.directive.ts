...
/**************** */
  ...
  private pasteData(pastedContent: string): void {
    ...
    if (this.plus) {
      this.hasPlus =
        this.inputElement.value.indexOf(this.plusSymbol) > -1;
    }
    /**************** */
  }
  ...
  private sanitizeInput(input: string): string {
    ...
    /******************** */
    else if (this.plus && this.isValidPlus(input)) {
      const regex = new RegExp(`[^0-9${this.plusSymbol}]`, 'g');
      result = input.replace(regex, '');
    }else   if (this.decimal && this.isValidDecimal(input)&& this.plus && this.isValidPlus(input)) {
      const regex = new RegExp(`[^0-9${this.decimalSeparator}${this.plusSymbol}]`, 'g');
      result = input.replace(regex, '');
    }
    /***************************** */
     else {
  ...
  private isValidDecimal(stringInput: string): boolean {
    if (!this.hasDecimalPoint) {
      return stringInput.split(this.decimalSeparator).length <= 2;
    } else {
      ...
      if (selectedText && selectedText.indexOf(this.decimalSeparator) > -1) {
        ...
        return stringInput.indexOf(this.decimalSeparator) < 0;
      }
    }
  }
/************************** */
private isValidPlus(stringInput: string): boolean {
  if (!this.hasPlus) {
    return stringInput.split(this.plusSymbol).length <= 2;
  } else {
    // the input element already has a decimal separator
    const selectedText = this.getSelection();
    if (selectedText && selectedText.indexOf(this.plusSymbol) > -1) {
      return stringInput.split(this.plusSymbol).length <= 2;
    } else {
      return stringInput.indexOf(this.plusSymbol) < 0;
    }
  }
}
/************************** */
